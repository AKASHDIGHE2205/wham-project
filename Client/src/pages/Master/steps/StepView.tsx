    ...
    <div className="min-h-screen bg-white border border-indigo-300 m-1 rounded-md p-2 sm:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
          <div>
            <h1 className="text-lg sm:text-xl font-bold text-indigo-600">Steps Master</h1>
          </div>
        </div>
                ...
                {loading ? (
                  ...
                  data?.map((step: Steps) => (
                    <tr key={step?.id} className="hover:bg-indigo-50 transition-colors duration-150">
                      <td className="px-2 py-2 text-center whitespace-nowrap text-sm text-black">{step?.id}</td>
        ...
        <div className="block md:hidden">
          {loading ? (
            ...
            <div className="space-y-3">
              {data?.map((step: Steps) => (
                <div key={step?.id} className="bg-white rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                  {/* Main Content Row */}
                  <div className="p-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <h3 className="font-semibold text-gray-900 text-sm">{step?.step_name}</h3>
                            <p className="text-xs text-gray-500 mt-0.5">ID: {step?.id}</p>
                          </div>
                        ...
                        {step?.step_desc && (
                          <p className="text-xs text-gray-500 mt-2 flex items-center gap-1">
                            ...
                            <span className="truncate">{step?.step_desc}</span>
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-1 mt-3 pt-2 border-t border-gray-50">
                      ...
                      <button
                        ...
                        title={step?.status === "A" ? "Deactivate" : "Activate"}
                      >
