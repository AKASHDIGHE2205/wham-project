...
const CollegeView = () => {
  ...
  const handleImageShow = (data: College) => {
    ...
    <div className="min-h-screen bg-white border border-orange-300 m-1 rounded-md p-2 sm:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
          <div>
            <h1 className="text-lg sm:text-xl font-bold text-orange-600">College Master</h1>
            <p className="text-orange-400 mt-1 sm:mt-2 text-sm sm:text-base">Manage Colleges.</p>
          </div>
        </div>
            ...
            <table className="w-full min-w-[500px]">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (
                  ...
                  data?.map((item) => (
                    <tr key={item?.clg_id} className="hover:bg-orange-50 transition-colors duration-150">
                      <td className="px-2 py-2 text-center whitespace-nowrap text-sm text-black">{item?.clg_id}</td>
      ...
      {showImage && (
        <ShowMedia show={showImage} setShow={setShowImage} data={selectedCollge} />
      )}
    </div>
  )
}
