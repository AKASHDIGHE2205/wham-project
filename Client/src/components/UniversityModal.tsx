...
const UniversityModal: FC<Props> = ({ show, setShow }) => {
  ...
  const filteredUniversities = userData?.filter((user) =>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                Choose a university from the list below
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              <p className="text-gray-400 text-sm mt-1">
                {search ? "Try adjusting your search terms" : "No active university found"}
              </p>
            </div>
          ) : (
          ...
          {!loading && filteredUniversities?.length > 0 && (
            <div className="mt-4 text-sm text-gray-500">
              Showing {filteredUniversities?.length} of {data?.length} universities
            </div>
          )}
        </div>
