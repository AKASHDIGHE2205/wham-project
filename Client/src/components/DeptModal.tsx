...
import { getActiveDepartment } from "../services/master/masterApi";

interface Props {
  show: boolean;
  setShow: (show: boolean) => void;
  onSelectDepartments: (departments: Department[]) => void;
...
const DeptModal: FC<Props> = ({
  ...
  const [tempSelectedDepartments, setTempSelectedDepartments] = useState<Department[]>(selectedDepartments);
  ...
  const handleSave = () => {
    onSelectDepartments(tempSelectedDepartments);
    setShow(false);
  };
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-linear-to-br from-orange-500 to-purple-600 rounded-xl flex items-center justify-center">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-university-icon lucide-university text-white">
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              <p className="text-gray-400 text-sm mt-1">
                {search ? "Try adjusting your search terms" : "No active departments found."}
              </p>
            </div>
          ) : (
          ...
          {!loading && filteredDepartments?.length > 0 && (
            <div className="mt-4 text-sm text-gray-500">
              Showing {filteredDepartments?.length} of {data?.length} departments
            </div>
          )}
        </div>
